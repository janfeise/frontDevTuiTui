...
const NavBar = (props) => {
  ...
  const { userAccount, TOKEN, userIdentity, setUserIdentity } =
  ...
  const getActiveIndex = () => {
    ...
    if (path === "/") return 0 /* 主页 */;
    else if (path === "/findJob") return 1 /* 找工作 */;
    else if (path === "/JobCircle") return 2 /* 招聘圈 */;
    else if (path === "/userSpace") return 5 /* 个人中心 */;
  };

  // 实现: 点击事件状态存储, 默认为0, 0表示主页
  const [activeIndex, setActive] = useState(() => getActiveIndex());
  ...
  const handleUserDropMenu = (index, text) => {
    ...
    switch (text) {
      case "个人中心":
        navigate("/userSpace?tab=info"); // 我的主页
        break;
      case "发布职位":
        navigate("/userSpace?tab=publishJob"); // 发布职位
        break;
      case "我的简历":
        navigate("/userSpace?tab=resume"); // 我的简历
        break;

      default:
        props.handleLogout(); // 登出
        break;
    }
  };
  ...
  const handleClick = (index, text) => {
    ...
    // 个人中心跳转
    if (text === "个人中心") {
      navigate("/userSpace?tab=info");
    }
  };

  useEffect(() => {
    if (userIdentity === "1") {
      setUserMenuOptions((prev) => {
        // 如果已经包含 "发布职位"，则不重复插入
        if (prev.includes("发布职位")) return prev;
        // 复制数组，避免直接修改状态
        const newOptions = [...prev];
        // 在第 1 个位置插入 "发布职位"
        newOptions.splice(1, 0, "发布职位");
        return newOptions;
      });
    }
  }, [userIdentity]);
    ...
    <header>
      <div className="u-flex u-flex-spaceBetween u-flex-alignCenter NavBar">
          ...
          {NAVBAR_ITEMS.map((text, index) => (
            <div key={index}>
              <UnderLineDiv
                ...
                index={index}
                specialIndex={activeIndex}
                specialClassName="nav__items--active"
                text={text}
                fontSize={activeIndex === index ? "var(--l)" : ""}
        ...
        <div className="right work">
          ...
          >
            <div
              className="u-flex u-flex-spaceBetween u-flex-alignCenter profile-container"
            ...
            >
              ...
              {/* <span className="profile-account">账号：{userAccount}</span> */}
              {/* </div> */}
            </div>
            {dropMenuState[5] && (
              /* 下拉菜单 */
              <div className="dropdown-menu">
                <DropMenu
                  ...
                  specialIndex={userMenuOptions.length - 1}
                  specialClassName="nav__log-out"
